...
我们已经完整阅读了整个框架，并对开发计划、整体架构都进行了详细的讨论。由于存储模块与元数据存储模块的高度相关性，我们将两者结合在一起考虑，本阶段提交的代码已经有元数据存储模块的成分，不过本文档暂时只涉及存储模块的设计。
...
参数：主键entry
返回值：entry对应的行
实现：以entry为key调用B+树的Row get(key)接口
...
2. 目前实现的增、删、改接口都以需要操作的Row类型数据为参数，之后会根据需要添加以索引为参数的相应接口
3. 目前实现的查找只是针对主键查找某一元组，其他形式的查找，如根据非主键查找、范围查找会在实现查询模块时根据需求添加

### 测试

已完成对Table类各个方法以及序列化类各个方法的测试，在TableTest和PersistentStorage中。
